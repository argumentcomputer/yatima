...
pub fn equal(a: &Value, b: &Value, dep: Index) -> bool {
  ...
  // TODO: Return true if they are proof irrelevant terms or are elements of a unit type
  match(a, b) {
	...
	(Neutral::Const(a_cnst, _), Neutral::Const(b_cnst, _)) =>
          // Univs should be equal since they have the same type. Also, because of CID <-> Pointer
          // correspondence, we need only check for pointer equality of constants
