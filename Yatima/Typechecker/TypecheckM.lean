...
def PrimConstOp.toPrimOp : PrimConstOp → PrimOp
    ...
    | .lit (.natVal v), .lit (.natVal v') =>
      if h : v < v' then do
        pure $ .some $ .app (.const `Decidable.isTrue (← primIndex .decT) []) $
          [.mk {proof? := true} $ .mk fun _ => .litProp $ .natLt v v' h]
      else do
        pure $ pure $ .app (.const `Decidable.isFalse (← primIndex .decF) []) $
          [.mk {proof? := true} $ .mk fun _ => .litProp $ .natNLt v v' h]
    | _, _ => pure none
  | .natDecLe => .mk fun vs => do
    ...
    | .lit (.natVal v), .lit (.natVal v') =>
      if h : v ≤ v' then do
        pure $ .some $ .app (.const `Decidable.isTrue (← primIndex .decT) []) $
          [.mk {proof? := true} $ .mk fun _ => .litProp $ .natLe v v' h]
      else do
        pure $ pure $ .app (.const `Decidable.isFalse (← primIndex .decF) []) $
          [.mk {proof? := true} $ .mk fun _ => .litProp $ .natNLe v v' h]
    | _, _ => pure none
