...
def cacheAndSetConst (cid : Ipld.Both Ipld.ConstCid) (constIdx : ConstIdx)
    (const : Const) : ConvertM Unit := do
  ...
  partial def constFromIpld (cid : Ipld.Both Ipld.ConstCid) :
    ...
    | none =>
      withResetBindDepth do
        ...
        | .ok primOpMap =>
          match primOpMap.find? cid.anon with
          | some const =>
            cacheAndSetConst cid constIdx const
            return constIdx
          | none =>
